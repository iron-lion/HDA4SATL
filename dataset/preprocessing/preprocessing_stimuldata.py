...
def _get_data(mo):
    ...
    print('gene', len(adata_subset.obs_names))
    print('gene', len(adata_subset.var_names))
    final_data = pd.DataFrame(adata_subset.X, columns=adata_subset.var_names)
    final_data = final_data.T
    final_data.to_csv(f'norm_{mo}_unst.csv')
    ...
    final_data = pd.DataFrame(adata_subset.X, columns=adata_subset.var_names)
    final_data = final_data.T
    final_data.to_csv(f'norm_{mo}_lps2.csv')
    ...
    final_data = pd.DataFrame(adata_subset.X, columns=adata_subset.var_names)
    final_data = final_data.T
    final_data.to_csv(f'norm_{mo}_lps4.csv')
    ...
    final_data = pd.DataFrame(adata_subset.X, columns=adata_subset.var_names)
    final_data = final_data.T
    final_data.to_csv(f'norm_{mo}_lps6.csv')
    return adata
