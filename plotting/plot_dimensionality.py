...
def _plot(args):
    ...
    PATH_fig4 = os.path.join('./results/satl_dimensionality/')

    # create dataframe giving an overview of all files in path
    file_list = []
    for (_, _, filenames) in os.walk(PATH_fig4):
        [file_list.append(f) for f in filenames if ('_ndim_' in f) and ('_h' in f)]
        break
    ...
    for k, curve in c:
        ...
        # plot
        # ------------
        #ax.plot(curve['dim'], curve['h'], lw=2, label=k[0]+'-'+k[1])#, color='blue')
    ...
    ax.set_xlabel('Dimensionality', fontsize=18)
    ax.set_ylabel('h-score', fontsize=18)


    plt.tight_layout()
    plt.savefig(f'./plots/satl_dim_test.png')
