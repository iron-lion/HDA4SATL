...
def main(n_jobs:int, scenario:dict):
    ...
    external = 'norm_muraro'

    latent_space = PCA(n_components=pca_dim)
    dataset, label_set = baron_external_load_total(root_dir, external, latent_space)
    ...
    print(set(dataset['y_ind_test']))
    run_all = SATL(dataset,
                    out_dir,
                    2, 
                    dim=ndim, 
                    alpha=[0.01, 0.05, 0.1, 1, 10, 100],
                    n_resample_source=oversample, n_resample_target=-1, n_jobs = n_jobs, K=thisK, feature_analysis=False)
    run_all.run_mode_indiv()
...
if __name__ == '__main__':
    ...
    logging.FileHandler(f'./satl_baron_jobtime_{current_time}.log')
    ...
    scenarios.append({'seed': 37, 'thisK': 10, 'oversample': 300, 'ndim': 11, 'num_to_del': 2, 'pca_dim': 50})
    #scenarios.append({'seed': 7, 'thisK': 10, 'oversample': -1, 'ndim': 11, 'num_to_del': 2, 'pca_dim': 50})
